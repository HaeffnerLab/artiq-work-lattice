...
class AD9910RAM(EnvExperiment):
    ...
    def build(self): #this code runs on the host computer
        ...
        self.ttl6 = self.get_device("blue_PIs")                                         #sets ttl channel 6 device drivers as attributes
        self.u = self.get_device("urukul1_ch2")                             #sets urukul 0, channel 1 device drivers as attributes and renames object self.u
    ...
    def run(self):
        ...
        for i in range(len(data)//2):                                       #splits list into 2 halves and defines each separately
            ...
            data[i + len(data)//2] = 0x3fff << 18                           #second half holds maximum amplitude
        ...
        self.u.set_profile_ram(                                             #sets profile in RAM to be used
            start=0, end=0 + len(data) - 1, step=10000 | (2**6 - 1 ) << 18,                         #start/end give addresses of ends of ram data, step gives step length
        ...
        #write to cfr
        self.u.set_cfr1(ram_enable=1, ram_destination=RAM_DEST_ASF, osk_enable=0)         #writes to CFR1 (control function register 1)
        ...
        self.u.set_amplitude(1.0)#sets frequency
        self.u.cpld.io_update.pulse_mu(8)                                   #I think this clocks all the CPLD registers so they take the values written to them
