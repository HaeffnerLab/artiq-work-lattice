...
class RabiExcitationWithStarkShift:
    ...
    def subsequence(self):
        r = RabiExcitationWithStarkShift
        self.get_729_dds(r.channel_729)
        ...
        if r.stark_shift_on:
            self.dds_729_SP_bichro.set(
                            sp_freq_729 + r.stark_shift_detuning,
                            amplitude=r.stark_shift_amp, ref_time_mu=r.phase_ref_time 
                        )
            self.dds_729_SP_bichro.set_att(r.stark_shift_att) 
        delay(5*us)  # make sure it has time to be programmed
